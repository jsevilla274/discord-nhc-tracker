    ...
    rssData.channel.item.forEach(item => {
        ...
        if (cyclone.hasOwnProperty('wind') && cyclone.hasOwnProperty('type')) {
            ...
            if (cyclone.type.toLowerCase() === 'hurricane') {
                let windMPH = 0;
                try {
                    windMPH = cyclone.wind.match(/\d+/)[0];
                } catch (error) {}
 ...
 * @returns {String}
 */
export function getCycloneConeImageLink(cycloneSeasonWalletId, cycloneAtcfId) {
    return `https://www.nhc.noaa.gov/storm_graphics/${cycloneSeasonWalletId}/${cycloneAtcfId}_3day_cone_with_line_and_wind.png`;
}
